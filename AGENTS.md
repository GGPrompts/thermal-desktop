...
Keep sibling repos aligned when changes affect setup or generated artifacts. Deployment configs (Hyprland keybinds, kitty theme, systemd units, Claude hooks) now live in `deploy/` within this repo (`deploy/hypr/hyprland.conf`, `deploy/kitty/kitty.conf`, `deploy/hooks/state-tracker.sh`, `deploy/systemd/`). Sync `~/projects/thermal-os-dotfiles` for shell/editor configs that remain there (zshrc, nvim, starship). Sync `~/projects/ggl` when changing `.ggl` schemas or codegen expectations; `crates/thermal-core/Cargo.toml` uses a path dependency on `~/projects/ggl/proto/ggl-build`, and local agent tooling expects the `ggl` MCP server configured in `.claude/settings.local.json`.
