...
//!
//! Each session writes to `/run/user/$UID/thermal/sessions/<id>.events.jsonl`.
...
impl EventLog {
    ...
    ///
    /// Path: `$XDG_RUNTIME_DIR/thermal/sessions/<session_id>.events.jsonl`
...
mod tests {
    ...
    fn write_and_read_events() {
        let dir = std::env::temp_dir().join("thermal-event-log-test");
    ...
    fn rotation_truncates() {
        let dir = std::env::temp_dir().join("thermal-event-log-rotate-test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
