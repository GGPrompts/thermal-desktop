...
impl<T> Term<T> {
    ...
    /// Adjust a previous cell when adding a zero-width character changes the
    /// width of the full grapheme sequence.
    fn update_zerowidth_sequence_width(&mut self, point: Point, new_width: usize)
    where
        T: EventListener,
    {
        let old_width = if self.grid[point.line][point.column]
            .flags
        ...
        match (old_width, new_width) {
            (1, 2) => self.expand_zerowidth_sequence(point),
            (2, 1) => self.shrink_zerowidth_sequence(point),
            _ => {}
        }
    }
    ...
    where
        T: EventListener,
    {
        if self.grid.cursor.input_needs_wrap || point.column + 1 >= self.columns() {
            return;
        }
    ...
    /// Collapse a wide grapheme back to a single cell after a zero-width
    /// modifier changes its display width.
    fn shrink_zerowidth_sequence(&mut self, point: Point)
    where
        T: EventListener,
    {
        if !self.grid[point.line][point.column]
            .flags
            .contains(Flags::WIDE_CHAR)
            || point.column + 1 >= self.columns()
        {
            return;
        }
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            let new_width = {
                ...
                let mut cluster =
                    String::with_capacity(2 + cell.zerowidth().map_or(0, |extra| extra.len()));
                cluster.push(cell.c);
            ...
            self.update_zerowidth_sequence_width(Point::new(line, column), new_width);
            return;
        }
